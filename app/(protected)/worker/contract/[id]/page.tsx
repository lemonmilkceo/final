...
const SAMPLE_CONTRACTS = {
  'sample-pending': {
    ...
    expires_at: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toISOString(), // D-2
...
export default async function ContractDetailPage({
  ...
  // 게스트 모드: 샘플 데이터 반환
  if (isGuest) {
    const sampleContract = SAMPLE_CONTRACTS[id as keyof typeof SAMPLE_CONTRACTS];
    
    if (!sampleContract) {
      // 샘플에 없는 ID면 대기중 샘플로 대체
      return <WorkerContractDetail contract={SAMPLE_CONTRACTS['sample-pending']} isGuestMode={true} />;
    }
