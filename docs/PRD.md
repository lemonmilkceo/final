...
## 📝 Amendment 28: 관리자 및 결제 UX 개선 (2026년 2월 10일)

> **버전**: 1.28  
> **변경 사유**: 관리자 페이지 데이터 정확성 및 프로모션 정책 개선

### 28.1 사용자 목록 크레딧 표시 수정

#### 문제점

- 사용자 목록 페이지에서 크레딧이 잘못 표시됨
- 상세 페이지에서는 올바른 크레딧 표시

#### 원인

- 목록 페이지: `credits (amount)` 조인으로 첫 번째 크레딧 행 가져옴
- credits 테이블에 여러 타입(`contract`, `ai_review`) 존재
- 상세 페이지: `credit_type = 'contract'`로 필터링

#### 해결

| 항목 | 기존 | 변경 |
|------|------|------|
| 크레딧 조회 | `credits (amount)` 조인 | 별도 쿼리로 `credit_type = 'contract'` 필터링 |
| 표시 값 | 첫 번째 크레딧 행의 amount | contract 타입 크레딧만 표시 |

### 28.2 프로모션 코드 무제한 사용 제거

#### 변경 사항

| 항목 | 기존 | 변경 |
|------|------|------|
| 최대 사용 횟수 | 선택사항 (비워두면 무제한) | **필수** 입력 |
| 기본값 | 빈 문자열 | `1` |
| 유효성 검사 | 없음 | 1 이상 숫자 필수 |

#### UI 변경

- 라벨에 필수 표시 (`*`) 추가
- 안내 문구: "이 코드를 사용할 수 있는 최대 횟수입니다"
- "비워두면 무제한" 안내 제거
...
## 📝 Amendment 29: 크레딧 및 결제 정책 변경 (2026년 2월 10일)

> **버전**: 1.29  
> **변경 사유**: 수익 모델 최적화, 단건 결제 환불 악용 방지
...
| 7일 이내 + 미사용 | 0% (면제) | 0% (면제) |
| 그 외 | **10%** | **15%** |
| 최소 환불 금액 | 1,000원 | 1,000원 (유지) |

#### 변경 이유
- **단건 결제 악용 방지**: 5건 구매 후 4건 환불 시 손해 방지
- 계산 예시:
  - 5건 4,900원 구매 → 1건 사용 → 4건 환불
  - 환불 금액: 3,920원 × (1 - 0.15) = **3,332원**
  - 실질 1건 비용: 4,900 - 3,332 = **1,568원**
  - 단건 직접 구매: **1,500원** → **단건 구매가 유리**

#### 영향 범위
- `app/api/refund/request/route.ts` (FEE_RATE 상수)
- `components/payment/RefundRequestSheet.tsx` (안내 문구)
- `app/(public)/refund/page.tsx` (환불 정책 페이지)
- `refund_requests.fee_rate` 기본값 (DB)
...
## 📝 Amendment 30: Enterprise 문의 및 실시간 채팅 시스템 (2026년 2월 10일)

> **버전**: 1.30  
> **변경 사유**: B2B 고객 유치 및 계약 당사자 간 커뮤니케이션 기능 추가

### 30.1 Enterprise/구독 문의 안내

#### 기능 설명
- 가격 페이지에서 대량 구매/정기 구독 문의 유도
- 기존 1:1 문의 시스템 활용 (enterprise 카테고리 추가)

#### UI/UX
| 위치 | 내용 |
|------|------|
| 가격 페이지 | 상품 목록 아래 Enterprise 안내 카드 |
| 문의 유형 | "기업/구독 문의" 카테고리 추가 |

#### 플로우
1. 가격 페이지 → "기업/구독 문의하기" 클릭
2. `/support?category=enterprise`로 이동
3. 로그인 상태면 바로 `/support/inquiry/new?category=enterprise`로 리다이렉트
4. 문의 유형이 자동 선택된 상태로 폼 표시

### 30.2 실시간 채팅 시스템

#### 기능 설명
- 서명 완료된 계약서 기반 1:1 채팅
- 사업자-근로자 간 파일/메시지 교환
...
|------|------|
| 텍스트 메시지 | 실시간 전송/수신 |
| 파일 첨부 | 이미지, PDF, 문서 (최대 20MB) |
| 읽음 표시 | 읽지 않은 메시지 카운트 |
| 실시간 알림 | Supabase Realtime 구독 |

#### 진입점
- 서명 완료된 계약서 상세 페이지 → 채팅 버튼
- 채팅 목록 페이지 (`/chat`)
...
| 테이블 | 설명 |
|--------|------|
| `chat_rooms` | 채팅방 (계약서 1:1 매핑) |
| `chat_messages` | 메시지 (텍스트/파일) |

#### API 엔드포인트
| Endpoint | Method | 설명 |
|----------|--------|------|
| `/api/chat/rooms` | GET | 내 채팅방 목록 |
| `/api/chat/rooms` | POST | 채팅방 생성 |
| `/api/chat/rooms/[id]/messages` | GET | 메시지 목록 |
| `/api/chat/rooms/[id]/messages` | POST | 메시지 전송 |
| `/api/chat/rooms/[id]/read` | POST | 읽음 처리 |

#### UI 컴포넌트
| 컴포넌트 | 설명 |
|----------|------|
| `ChatRoom` | 채팅방 UI (메시지 목록, 입력창) |
| `ChatButton` | 채팅 진입 버튼 (icon/primary/secondary) |
| `ChatList` | 채팅방 목록 |

### 30.3 Storage 설정 필요

채팅 파일 업로드를 위해 Supabase Storage 버킷 생성 필요:
...
계약서 작성 시 특약사항을 추가할 수 있는 기능입니다. 예시 선택과 직접 작성 모두 지원합니다.

### 31.2 변경 사항

#### 데이터베이스
- `contracts` 테이블에 `special_terms` (TEXT, nullable) 컬럼 추가

#### UI/UX (Step 9: 업무내용 화면)
1. **섹션 제목**: "추가하고 싶은 특약이 있나요?"
2. **안내 문구**: "선택사항이에요. 예시를 선택하거나 직접 작성할 수 있어요."
3. **특약 예시 태그 (토글 방식)**:
   - 수습 기간 3개월 적용
   - 비밀유지 의무 동의
   - 경쟁업체 취업 제한 (퇴직 후 1년)
   ...
   - 차량 사용 시 유류비 지원
   - 식대 별도 지급
4. **직접 작성 textarea**: 예시와 별도로 추가 내용 입력 가능

#### PDF 출력
- 특약사항이 있는 경우 별도 섹션으로 표시
- 개행 문자로 구분된 각 특약 항목 표시

### 31.3 구현 완료 항목
- [x] DB 마이그레이션 (`special_terms` 컬럼 추가)
- [x] Store 수정 (`contractFormStore.ts`)
- [x] Validation 수정 (`validation.ts`)
- [x] Step9 UI 수정 (특약 예시 태그 + 직접 작성 textarea)
- [x] PDF 템플릿 수정 (`ContractPDF.tsx`, `contract-template.tsx`)
- [x] 미리보기/상세 페이지 수정
- [x] 문서 업데이트
